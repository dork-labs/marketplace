...
- **R8. Lifecycle.** The ledger is overwrite-only and never grows; history goes to the journal as sampled `usage.snapshot` events. When an account leaves the registry, DorkOS deletes its ledger file, `flow usage prune` removes any left behind (`default` only once its runtime has registered accounts; the id `default` is reserved), and `flow accounts` drops its `fleet.json` policy with a note. A reading past its reset is read as empty, never deleted early.
