...
- **Adopt before releasing:** a pending slot, or a `queued` run with no handle, older than the start timeout, is checked for its minted session first (cli and cmux: `sessions/*.json` and the transcript under the account dir; DorkOS: `GET /api/sessions/<id>`). Found → adopted into the slot. Not found → the slot is cleared (a `queued` worker's claim is released to ready). **Except on DorkOS through `session_start`**, where the server mints the id, so "not found" proves nothing: that run parks instead ("the supervisor stopped while starting a DorkOS session; check DorkOS for a session in <worktree> whose context names flow id <minted id>, then `flow release` or re-run"). The `seedContext` carries the minted id for exactly this search. A live session is never orphaned into a second writer.
