...
| 3   | Worktrees                             | Look in the current checkout, then the main checkout                                               | An ignored file never reaches a new worktree; the main checkout is the project's home.                                                                   |
...
| 6   | Migration                             | Automatic at the `/flow` first-run guard and `/flow:init`, copy-only, never deletes the old files | A shared install may still serve another project from the old file; copying is idempotent and safe to repeat.                                            |
