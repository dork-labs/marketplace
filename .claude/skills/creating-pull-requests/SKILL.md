...
| Branch up to date        | Required (`strict: true`) until the merge queue is on: a PR behind `main` waits until it is updated         |
| Force push to `main`     | Not allowed                                                                                                |
| Merge style              | Squash via PR                                                                                              |
| Auto-merge               | On. Arm your own PR; nothing else arms PRs yet                                                              |
| Merge queue              | Being switched on (a repository ruleset like dork-labs/dorkos's); check `rulesets` above                   |
...
It joins the merge queue and merges as soon as the required checks pass there. The queue tests your PR on top of `main` and everything ahead of it, so being behind blocks nothing: never update a branch to satisfy a gate. Until the queue is on, `main` requires branches to be up to date instead, and a PR that falls behind sits armed until you run `gh pr update-branch <number>`. Never admin-merge past a red or missing check.

Arm only when every signal is good; `scripts/should-arm-automerge.sh` (pinned by `scripts/test-should-arm-automerge.sh`) is the rule, so never a draft, a conflicting PR, a PR with changes requested or an unresolved thread, or one with a check failing, cancelled or still running. **A PR labelled `hold`, `do-not-merge`, `wip` or `blocked` is not armed**; the labels mean the same in dork-labs/dorkos. Nothing arms PRs automatically yet: a scheduled `merge-tail` workflow that runs that rule over every open PR arrives once the `dorkos-merge-tail` GitHub App is set up here. A label does not disarm a PR that is already armed: `gh pr merge --disable-auto <number>` does.
