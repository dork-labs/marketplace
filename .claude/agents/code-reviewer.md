...
- **No committed local config or secrets** — `.agents/flow/config.local.json`, any other `*.local.json`, and a legacy `plugins/flow/config/config.json` stay gitignored; only the `*.example.json` templates are committed. No tokens, API keys, personal paths, or workspace ids in committed files.
