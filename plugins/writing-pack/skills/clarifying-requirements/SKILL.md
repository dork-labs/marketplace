...
- **Use a structured question tool for bounded choices** (in Claude Code, `AskUserQuestion`) — see Recommendation Discipline below
...
You have the codebase, research findings, existing patterns, and domain knowledge. Use them to form a point of view on every question. The user asked you to think, not just to enumerate options.
    ...
    - question: "Where should the new settings panel live?"
      ...
      options:
        - label: "Extend existing SettingsDialog (Recommended)"
          description: "SettingsDialog already handles config. Adding a tab keeps navigation consistent with the existing 3-tab layout."
...
When planning, apply this skill BEFORE exploring:

```
❌ Wrong order:
   Enter plan mode → Explore everything → Ask questions → Re-explore

✅ Right order:
   Enter plan mode → Ask key questions → Explore with focus → Present plan
```
...
If a request is vague, clarify BEFORE ideation or writing a spec — ideation works better with clear inputs. Specifications especially must not be built on assumptions.
...
In an unattended or scheduled run, do not block waiting for a reply. Pick the recommended option, write down the assumption where the next reader will see it (the PR, the ticket, a log), and keep going. Stop only when the choice is hard to reverse.
...
- **Your planning workflow.** If you use an ideation or spec stage (for example a `/flow:ideate` command), name it in "Before Ideation and Specification" so clarification happens before it runs.
- **What always needs a human.** List the decisions an agent should never assume in your project, such as anything outward-facing, destructive, involving money or secrets, or changing scope.
- **Where assumptions get recorded.** Say where an unattended run writes the assumptions it made: the PR body, a ticket comment, or a log file.
- **Your own examples.** Replace the settings-panel and storage examples with a real question from your codebase.
