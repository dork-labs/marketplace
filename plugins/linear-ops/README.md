...
- An offer to set up a "Linear Tender" agent. This agent checks your Linear inbox and acts on what it can. You choose whether to add it. Nothing is created without your OK.
- A check that looks at your Linear inbox every 15 minutes, once the agent is set up.

## The inbox check waits for the agent

The 15-minute check needs the Linear Tender agent to run it. If you have not set up that agent yet, DorkOS creates the check but leaves it off. It turns on once the agent exists. That way nothing runs before you say yes.
...
- A template for the Linear Tender agent, built on the `/flow` tending and Linear skills.
