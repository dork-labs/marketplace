...
- When an account is no longer listed, `flow accounts` removes its leftover settings and tells you. The new `flow usage prune` deletes its usage file (`--dry-run` shows what would go). Neither ever touches a runtime's `default`.
