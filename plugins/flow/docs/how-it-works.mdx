...
<Cards>
  ...
  <Card title="The tracker-adapter seam">
...
inbound signal ─▶ InboundTransport ─▶ TrackerEvent[] ─▶ reconciler (re-reads truth via the adapter)
                                              ...
                                              calls a pure decision oracle
                                                              │
                                                              ▼
                                              acts via the tracker adapter (claim, comment, transition, …)
```
...
<Step>drop, with a warning, any entry whose `occurredAt` is not an ISO-8601 date-time with a zone or is more than an hour in the future, or that has no comment, then keep only entries strictly newer than the `since` watermark (`occurredAt > since`, compared as instants, exclusive);</Step>
...
Because the watermark is exclusive and advances to the newest entry, a second `poll(watermark)` over the same snapshot returns `[]`: no event is ever re-emitted, and the cursor (persisted in the run record) keeps the stream gap-free across restarts. Only a real timestamp can become the watermark, so one malformed entry can never leave the inbox waiting for a time that never comes; a saved watermark that is not a timestamp is discarded and the whole inbox is read again.
...
interface FlowRun {
  ...
  startedAt;
  completedAt?;
  provenance?; // v/harness/session/account/worker/host/instance/surface/resumeUrl/worktree/branch
  // — where the run came from, and the block signed onto every outward write
}
```
...
## The tracker-adapter seam: the engine stays pure

Every fact so far has pointed at one rule (charter G8): **all tracker I/O is confined to the tracker adapter skill** — the shipped `skills/linear-adapter/`, or the one `/flow:init` generated into your project at `.agents/flow/adapters/<tracker>/`, whichever tracker your config names. Outside the shipped adapter the plugin contains zero tracker strings: no tracker MCP tool name, no CLI invocation, no tracker-specific field name. A grep guard enforces it across the skills, the commands, the engine scripts, and the loop hook.

This confinement is what makes everything above pure and portable:

- The **oracles** see only the normalized `WorkItem`, never a tracker-native issue.
