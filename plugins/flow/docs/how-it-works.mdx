...
<Step>drop, with a warning, any entry whose `occurredAt` is not an ISO-8601 date-time with a zone, then keep only entries strictly newer than the `since` watermark (`occurredAt > since`, compared as instants, exclusive);</Step>
