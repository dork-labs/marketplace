...
Write a `provenance` block onto this issue's record in `.dork/flow/flow-state.json`, so a later session can find its way back to this one. This is also the block that gets **signed outward** onto every body the run writes to a tracker or forge — the signature's wire format is defined once, by the adapter skill's "Provenance: signing outward writes" section. Fill in whatever the running harness can actually determine:

| Field        | What it is                                                                                                                                                                                                                                                                                                                                                                     |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `v`          | The signature's schema version. `1`. Always write it, since it is the one field that is not harness-derived.                                                                                                                                                                                                                                                                    |
| `harness`    | The agent harness you are running in, as one of `claude-code` \| `codex` \| `opencode` \| `other:<name>`. Use the `other:` form for anything else rather than stretching a known name to fit.                                                                                                                                                                                   |
...
| `account`    | A short, **non-PII** handle for the **harness** account this run authenticated as — the agent runtime's own sign-in, **never the tracker account** a tracker's "authenticated user" read returns (that one is usually a real person). Enough for a later reader to tell "same machine, different account" from "same session". **Derive it, never guess**: for `claude-code`, the basename of `CLAUDE_CONFIG_DIR`, defaulting to `claude` when it is unset; for `codex` and `opencode`, the same idea applied to their own profile / home directory (the one that harness resolves for itself). If you cannot derive it, **omit it**. **Never an email address** — these bodies land on trackers and public forges; **if the value you derived contains an `@`, omit `account` entirely** rather than cleaning it up. |
| `agentId`    | _(local only)_ The worker/agent id this run is operating as, when the harness assigns one.                                                                                                                                                                                                                                                                                     |
| `host`       | This machine's hostname. Load-bearing: a retained transcript is only reachable from the machine holding it.                                                                                                                                                                                                                                                                     |
| `instanceId` | The DorkOS install's UUID, **when this run is under DorkOS**. Omit it entirely outside DorkOS — it is what lets a reader tell this install's session from an identically-named one elsewhere. Do not substitute the hostname.                                                                                                                                                   |
| `surface`    | Where this run was driven from: `dorkos` \| `bare-cli` \| `ci`. **Always write it, on every surface** — it is not DorkOS-only. It selects which resume mechanism a later reader uses, and the `bare-cli` branch of that choice needs it exactly as much as the `dorkos` one.                                                                                                     |
| `resumeUrl`  | A URL that actually opens this session, when one exists **and this session is meaningfully resumable**. Omit it rather than writing a link that resolves to nothing.                                                                                                                                                                                                            |
| `worktree`   | _(local only)_ The absolute path this run is executing in (from 0.1, 0.2, or 0.3).                                                                                                                                                                                                                                                                                             |
| `branch`     | _(local only)_ The branch this run is on.                                                                                                                                                                                                                                                                                                                                      |
...
**The three `(local only)` rows stay on disk and never go on the wire.** The signature carries the other eight and no more: an absolute worktree path is `/Users/<real name>/…`, which must never land on a public forge, and a branch and a worker id route nothing a reader cannot already see. Writing the full block here and signing a subset outward is deliberate — the local record is for this machine, the signature is for everyone who can read the thread.
