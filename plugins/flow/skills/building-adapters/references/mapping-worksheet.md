...
[`<flow-root>/adapters/SPEC.md`](../../../adapters/SPEC.md) section 2.
...
| `size`           | native estimate field                                   | The union `number \| string`: emit points as a **number** (any scale), or a t-shirt string (`xs`-`xxl`) only if your tracker has no numeric estimate field. Pass the native shape through unconverted. Drives the dispatch size tier + sub-issue promotion, which compares `sizeOrdinal(size) >= sizeOrdinal(threshold)` — never `size >= "xl"`. Missing -> `undefined`, never `0` or smallest. |
