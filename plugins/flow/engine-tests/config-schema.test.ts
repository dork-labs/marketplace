...
describe('FlowConfigSchema — the drain block (flow-handoff-dispatch §6)', () => {
  ...
  it('round-trips every field set', () => {
    const drain = {
      ...
      pollSeconds: 10,
      waitIfResetWithinMinutes: 0,
      modelFallback: { 'claude-code': ['workhorse', 'fast'], codex: ['gpt-x'] },
      armAutoMerge: true,
      host: 'cmux',
  ...
  it.each([
    ...
    ['waitIfResetWithinMinutes below 0', { waitIfResetWithinMinutes: -1 }],
    ['waitIfResetWithinMinutes as a fraction', { waitIfResetWithinMinutes: 1.5 }],
    ['a modelFallback entry that is not a list', { modelFallback: { 'claude-code': 'fast' } }],
    ['an empty modelFallback model', { modelFallback: { 'claude-code': [''] } }],
